...
function triangleSet(points::Vector)
    triSet = Vector{Set{Int}}()
    for p in points
        p_len = length(p.list)
        for i in 1:p_len-1
            push!(triSet, Set([p.ind, p.list[i], p.list[i+1]]))
        end
        p.list[1] in points[p.list[end]].list && push!(triSet, Set([p.ind, p.list[1], p.list[end]]))
    end
    unique(triSet)
end

function boundary(p::Vector)
    b = [1]
    prev = 1
    point = p[prev].list[1]
    while point != 1
...
# Массив граничных треугольников с упорядоченными вершинами:
#  1-ый и 2-ый индексы - граничные узлы в поредяке обхода
#  3-ий - оставшийся индекс граничного треугольника
function boundarySet(triSet::Vector, b::Vector)
    N = size(b, 1)-1
    bSet = Matrix{Int}(undef,3,N)
    for i in 1:N
        bSet[1,i] = b[i]
        bSet[2,i] = b[i+1]
        for tri in triSet
            if b[i] in tri && b[i+1] in tri
                bSet[3,i] = setdiff(tri, Set(b[i:i+1])) |> only
                break
            end
        end
    end
...
function adjacencyMatrix(points::Vector)
    M = spzeros(Bool, size(points,1), size(points,1))
    for p in points
        for neghbour in p.list
            M[p.ind, neghbour] = true
            M[neghbour, p.ind] = true
        end
    end
    M
end

function plain((Ax,Ay), (Bx,By), (Cx,Cy), e::Int)
    X = [Ax, Bx, Cx]
    Y = [Ay, By, Cy]
    Z = zeros(3)
    Z[e] = 1
    Δ = det([X Y ones(3)])
    return det([Z Y ones(3)])/Δ, det([X Z ones(3)])/Δ, det([X Y Z])/Δ
end

function abc(i₁, i₂, i₃, e::Int, p::Vector)
    plain((p[i₁].x,p[i₁].y), (p[i₂].x,p[i₂].y), (p[i₃].x,p[i₃].y), e)
end

function S((x₁,y₁), (x₂,y₂), (x₃,y₃))
    abs(det([x₁-x₃ y₁-y₃;
             x₂-x₃ y₂-y₃])) / 2 ## |> abs
end

function Sw(i₁, i₂, i₃, p::Vector)
    S((p[i₁].x,p[i₁].y), (p[i₂].x,p[i₂].y), (p[i₃].x,p[i₃].y))
end
